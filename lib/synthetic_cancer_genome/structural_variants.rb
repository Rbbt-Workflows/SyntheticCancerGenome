...
module SyntheticCancerGenome
  ...
  def self.SV_regions(svs, copies = {})
    ...
    if Array === svs
      svs = svs.inject({}){|acc,e| id = Misc.digest(e); acc[id] = e; acc}
    end
  ...
  def self.SV_fragments(reference, svs)
    ...
    SyntheticCancerGenome.collect_fragments(reference, collect_fragment_ranges).each do |chr,sequences|
    ...
    removals = SyntheticCancerGenome.shift_ranges_by_inserts(removals, insertions)
  ...
  def self.place_SVs(sv_array)
    ...
    sv_array.each_with_index do |values,i|
  ...
  def self.apply_SVs_reference(reference, svs)
    ...
    reference = SyntheticCancerGenome.insert_fragments(reference,  insertions)

    reference = SyntheticCancerGenome.remove_fragments(reference,  removals)
  ...
  def self.transpose_mutations(svs, mutations)
    ...
    new_positions = SyntheticCancerGenome.duplicate_positions(mutations_chr, duplications)
    ...
    mutations_chr = SyntheticCancerGenome.shift_ranges_by_inserts(mutations_chr, insertions)
    ...
    mutations_chr = SyntheticCancerGenome.shift_ranges_by_deletes(mutations_chr, remove_fragments)
  ...
  def self.transpose_SVs(svs, new_svs, duplicate = false)
    ...
    new_positions = SyntheticCancerGenome.duplicate_positions(positions_chr, duplications)
    ...
    positions_chr = SyntheticCancerGenome.shift_ranges_by_inserts(positions_chr, insertions)
    ...
    positions_chr = SyntheticCancerGenome.shift_ranges_by_deletes(positions_chr, remove_fragments)
    ...
    new_svs.each do |id,values|
      ...
      new_source = choose(new_sources).split(":")
